...
nl = len(net_list)
all_weights = xgf.catch_feature_modify(net_list, nl)
all_max=xmc.max_feature(net_list)
# feature enhanced
...
for i in range(1, nl):
    for j in range(len(weight)):
        all_weights[i][j].data *= math.exp(1-all_max[i][j]/len(weight))
        weight[j].data += all_weights[i][j].data
